-- ============================================================================
-- MIGRATION: 20260923080000_seguranca_b3.sql
-- VERSÃO: v0.0.111 - SEGURANÇA RODADA B3
--
-- Item 1 — Empresa bloqueada:
--   Atualizar public.get_my_empresa_id() para retornar NULL quando a empresa
--   não estiver ativa (EXISTS ... AND (e.status IN ('ativa', 'ativo'))).
--   Preservar platform_admin (que não depende de get_my_empresa_id).
--
-- Item 2 — Permissões papel 'anon':
--   REVOKE ALL ON ALL TABLES/SEQUENCES/ROUTINES IN SCHEMA public FROM anon;
--   GRANT SELECT ON public.planos TO anon;
--
-- Item 3 — Storage (logos e produtos):
--   Ajustar file_size_limit de produtos para 2097152 (2MB) e allowed_mime_types
--   para ARRAY['image/png', 'image/jpeg', 'image/webp'].
--   Recriar policies de storage.objects para logos e produtos:
--     INSERT/UPDATE/DELETE exigindo (storage.foldername(name))[1] = get_my_empresa_id()::text
--     (permitindo bypass para platform_admin).
--     MANTER policy de SELECT pública para bucket_id IN ('logos', 'produtos').
--
-- Item 4 — Numeração por empresa e unicidade:
--   Adicionar constraints UNIQUE idempotentes:
--     uq_vendas_empresa_numero em public.vendas (empresa_id, numero)
--     uq_pedidos_empresa_numero em public.pedidos (empresa_id, numero)
--     uq_compras_empresa_numero em public.compras (empresa_id, numero)
--   Atualizar as RPCs:
--     - criar_compra
--     - criar_pedido
--     - finalizar_venda
--     - converter_pedido_em_venda
--     para gerar número via COALESCE((SELECT MAX(numero) ... WHERE empresa_id = v_empresa_id), 0) + 1
--     sob pg_advisory_xact_lock(hashtext('empresa_num_' || v_empresa_id::text))
--     e inserção com OVERRIDING SYSTEM VALUE.
-- ============================================================================

-- ============================================================================
-- PARTE 1: ITEM 1 — get_my_empresa_id() COM VALIDAÇÃO DE STATUS DA EMPRESA
-- ============================================================================
...
-- ============================================================================
-- PARTE 2: ITEM 2 — PERMISSÕES DO PAPEL anon (MINIMAL PRIVILEGE)
-- ============================================================================
...
-- Conceder apenas SELECT na tabela planos para visualização de preços/planos públicos
GRANT SELECT ON public.planos TO anon;

-- ============================================================================
-- PARTE 3: ITEM 3 — STORAGE: BUCKETS E POLICIES
-- ============================================================================

-- Ajustar configurações do bucket produtos (2MB, mimes de imagem)
UPDATE storage.buckets
SET file_size_limit = 2097152,
    allowed_mime_types = ARRAY['image/png', 'image/jpeg', 'image/webp']
WHERE id = 'produtos';

-- Garantir bucket logos com limite de 2MB e os mesmos mimes permitidos
UPDATE storage.buckets
SET file_size_limit = 2097152,
    allowed_mime_types = ARRAY['image/png', 'image/jpeg', 'image/webp']
WHERE id = 'logos';

-- Recriar policies de storage.objects
-- SELECT pública para logos e produtos
DROP POLICY IF EXISTS "storage_select_public_logos_produtos" ON storage.objects;
...
CREATE POLICY "storage_select_public_logos_produtos" ON storage.objects
  FOR SELECT TO public
  USING (bucket_id IN ('logos', 'produtos'));

-- INSERT para logos e produtos: pasta raiz deve ser o empresa_id do usuário logado (ou bypass para platform_admin)
DROP POLICY IF EXISTS "storage_insert_logos_produtos" ON storage.objects;
...
CREATE POLICY "storage_insert_logos_produtos" ON storage.objects
  FOR INSERT TO authenticated
  WITH CHECK (
    bucket_id IN ('logos', 'produtos')
    AND (
...
-- UPDATE para logos e produtos
DROP POLICY IF EXISTS "storage_update_logos_produtos" ON storage.objects;
...
CREATE POLICY "storage_update_logos_produtos" ON storage.objects
...
-- DELETE para logos e produtos
DROP POLICY IF EXISTS "storage_delete_logos_produtos" ON storage.objects;
...
CREATE POLICY "storage_delete_logos_produtos" ON storage.objects
...
-- ============================================================================
-- PARTE 4: ITEM 4 — UNIQUE (empresa_id, numero) E SEQUENCIAMENTO POR EMPRESA
-- ============================================================================

DO $$
BEGIN
  -- 4.1 Unique constraint em vendas (empresa_id, numero)
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'uq_vendas_empresa_numero'
  ) THEN
    ALTER TABLE public.vendas ADD CONSTRAINT uq_vendas_empresa_numero UNIQUE (empresa_id, numero);
  END IF;

  -- 4.2 Unique constraint em pedidos (empresa_id, numero)
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'uq_pedidos_empresa_numero'
  ) THEN
    ALTER TABLE public.pedidos ADD CONSTRAINT uq_pedidos_empresa_numero UNIQUE (empresa_id, numero);
  END IF;

  -- 4.3 Unique constraint em compras (empresa_id, numero)
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'uq_compras_empresa_numero'
  ) THEN
    ALTER TABLE public.compras ADD CONSTRAINT uq_compras_empresa_numero UNIQUE (empresa_id, numero);
  END IF;
END $$;

-- ----------------------------------------------------------------------------
-- 4.4 RPC: criar_compra com numeração isolada por empresa
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4.5 RPC: criar_pedido com lock consistente e OVERRIDING SYSTEM VALUE
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4.6 RPC: finalizar_venda com lock de empresa e numeração isolada
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4.7 RPC: converter_pedido_em_venda com numeração isolada por empresa
-- ----------------------------------------------------------------------------
