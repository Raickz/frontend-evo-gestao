-- Adicionar coluna pedido_id em vendas (nullable para compatibilidade com vendas antigas)
ALTER TABLE public.vendas ADD COLUMN IF NOT EXISTS pedido_id uuid;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'vendas_pedido_id_fkey' 
...
-- Índice UNIQUE parcial: permite múltiplos NULL mas garante que um pedido gere no máximo 1 venda
CREATE UNIQUE INDEX IF NOT EXISTS idx_vendas_pedido_id_unique 
  ON public.vendas(pedido_id) 
  WHERE pedido_id IS NOT NULL;

-- Atualizar RPC converter_pedido_em_venda
