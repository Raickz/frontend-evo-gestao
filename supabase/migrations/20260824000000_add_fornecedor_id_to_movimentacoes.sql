-- Adicionar coluna fornecedor_id em movimentacoes_estoque
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_movimentacoes_fornecedor'
    ) THEN
        ALTER TABLE public.movimentacoes_estoque
        ADD CONSTRAINT fk_movimentacoes_fornecedor
        FOREIGN KEY (fornecedor_id) REFERENCES public.fornecedores(id) ON DELETE SET NULL;
    END IF;
END $$;

-- Índice composto para consultas
CREATE INDEX IF NOT EXISTS idx_movimentacoes_empresa_fornecedor
ON public.movimentacoes_estoque(empresa_id, fornecedor_id);

-- Criar RPC public.registrar_entrada_estoque_por_fornecedor
